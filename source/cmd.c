...
void Cbuf_InsertText(const char *text) {
    ...
    len = strlen(text);
    if (cmd_text.cursize) {
        if (cmd_text.cursize + len + 1 > cmd_text.maxsize) Sys_Error("%s: overflow", __func__);
...
void Cmd_Exec_f(void) {
    ...
    if (cl_warncmd.value || developer.value) Con_Printf("execing %s\n", Cmd_Argv(1));
...
static struct cmdalias_s *Cmd_Alias_Find(const char *name) {
    struct cmdalias_s *ret = NULL;
    struct stree_node *n;

    n = STree_Find(&cmdalias_tree, name);
    if (n) ret = cmdalias_entry(n);
...
static struct cmd_function_s *Cmd_FindCommand(const char *name) {
    struct cmd_function_s *ret = NULL;
    struct stree_node *n;

    n = STree_Find(&cmd_tree, name);
    if (n) ret = cmd_entry(n);
...
============
*/
qboolean Cmd_Exists(const char *cmd_name) { return Cmd_FindCommand(cmd_name) != NULL; }

qboolean Cmd_Alias_Exists(const char *cmd_name) { return Cmd_Alias_Find(cmd_name) != NULL; }
...
===================
*/
static qboolean Cmd_ForwardCheckConnected(void) {
    if (cls.state < ca_connected) {
        Con_Printf("Can't \"%s\", not connected\n", Cmd_Argv(0));
        return false;
    }
    if (cls.demoplayback) return false;
...
void Cmd_ForwardToServer(void) {
    ...
    if (!Cmd_ForwardCheckConnected()) return;
...
struct stree_root *Cmd_ArgCompletions(const char *name, const char *buf) {
    ...
    if (cmd && cmd->completion) root = cmd->completion(buf);
...
int Cmd_CheckParm(const char *parm) {
    ...
    for (i = 1; i < Cmd_Argc(); i++)
        if (!strcasecmp(parm, Cmd_Argv(i))) return i;
