...
#ifdef PARANOID
    if (c < -128 || c > 127) Sys_Error("%s: range error", __func__);
#endif
...
#ifdef PARANOID
    if (c < 0 || c > 255) Sys_Error("%s: range error", __func__);
#endif
...
#ifdef PARANOID
    if (c < ((short)0x8000) || c > (short)0x7fff) Sys_Error("%s: range error", __func__);
#endif
...
float MSG_ReadAngle(void) { return MSG_ReadChar() * (360.0 / 256); }
...
int MSG_ReadControlHeader(void) {
    ...
    c = (net_message.data[msg_readcount] << 24) + (net_message.data[msg_readcount + 1] << 16) +
        (net_message.data[msg_readcount + 2] << 8) + net_message.data[msg_readcount + 3];
...
static void *SZ_GetSpace(sizebuf_t *buf, int length) {
    ...
    if (buf->cursize + length > buf->maxsize) {
        if (!buf->allowoverflow)
            Sys_Error("%s: overflow without allowoverflow set (%d > %d)", __func__,
                      buf->cursize + length, buf->maxsize);
        if (length > buf->maxsize) Sys_Error("%s: %d is > full buffer size", __func__, length);
        if (developer.value) /* Con_Printf may be redirected */
...
static const char *COM_FileExtension(const char *in) {
    ...
    dot++;
    for (i = 0; i < sizeof(exten) - 1 && *dot; i++, dot++) exten[i] = *dot;
    exten[i] = 0;
...
int COM_DefaultExtension(const char *path, const char *extension, char *out, size_t buflen) {
    ...
    if (strlen(out) + strlen(extension) + 1 > buflen) return -1;
...
skipwhite:
    ...
    if (c == '\"') {
        ...
        while (1) {
            ...
            if (len < sizeof(com_tokenbuf) - 1) com_tokenbuf[len++] = c;
        }
    }
    ...
    do {
        ...
        data++;
        c = *data;
        if (split_single_chars && strchr(single_chars, c)) break;
    } while (c > 32);
...
void COM_CheckRegistered(void) {
    ...
    for (i = 0; i < 128; i++)
        if (pop[i] != (unsigned short)BigShort(check[i])) Sys_Error("Corrupted data file.");
...
void COM_InitArgv(int argc, const char **argv) {
    ...
    for (com_argc = 0; (com_argc < MAX_NUM_ARGVS) && (com_argc < argc); com_argc++) {
        ...
        if (!strcmp("-safe", argv[com_argc])) safe = true;
    }
...
char *va(const char *format, ...) {
    ...
    if (len > COM_STRBUF_LEN - 1) Con_DPrintf("%s: overflow (string truncated)\n", __func__);
...
void COM_WriteFile(const char *filename, const void *data, int len) {
    ...
    if (!f) {
        ...
        f = fopen(name, "wb");
        if (!f) Sys_Error("Error opening %s", filename);
    }
    fwrite(data, 1, len, f);
    fclose(f);
}
...
static void COM_ScanDirDir(struct stree_root *root, DIR *dir, const char *pfx, const char *ext,
                           qboolean stripext) {
    int pfx_len, ext_len;
    struct dirent *d;
    char *fname;
...
static void COM_ScanDirPak(struct stree_root *root, pack_t *pak, const char *path, const char *pfx,
    ...
    for (i = 0; i < pak->numfiles; i++) {
        ...
        if (path && path_len) {
            if (strncasecmp(pak_f, path, path_len)) continue;
            if (pak_f[path_len] != '/') continue;
            pak_f += path_len + 1;
        }

        /* Don't match sub-directories */
        if (strchr(pak_f, '/')) continue;

        /* Check the prefix and extension, if set */
        if (pfx && pfx_len && strncasecmp(pak_f, pfx, pfx_len)) continue;
        if (ext && ext_len && !COM_CheckExtension(pak_f, ext)) continue;
...
============
*/
void COM_ScanDir(struct stree_root *root, const char *path, const char *pfx, const char *ext,
                 qboolean stripext) {
    searchpath_t *search;
...
static void *COM_LoadFile(const char *path, int usehunk, size_t *size) {
    FILE *f;
    byte *buf;
    char base[32];
    int len;
    ...
    if (!buf) Sys_Error("%s: not enough space for %s", __func__, path);
...
void *COM_LoadHunkFile(const char *path) { return COM_LoadFile(path, 1, NULL); }

void *COM_LoadTempFile(const char *path) { return COM_LoadFile(path, 2, NULL); }
...
void *COM_LoadStackFile(const char *path, void *buffer, int bufsize, size_t *size) {
    byte *buf;
...
static pack_t *COM_LoadPackFile(const char *packfile) {
    ...
    if (COM_FileOpenRead(packfile, &packhandle) == -1) return NULL;
    ...
    if (numfiles != ID1_PAK0_COUNT) com_modified = true;  // not the original file
...
void COM_Gamedir(const char *dir) {
    ...
    if (!strcmp(gamedirfile, dir)) return;  // still the same
    ...
    if (!strcmp(dir, "id1") || !strcmp(dir, "qw")) return;
    ...
    if (i) {
        ...
        while (++i < com_argc) {
            ...
            if (!strcmp(COM_FileExtension(com_argv[i]), "pak")) {
                ...
                if (!search->pack) Sys_Error("Couldn't load packfile: %s", com_argv[i]);
            } else
...
static const char *Info_ReadString(const char *infostring, char *buffer, int buflen) {
    ...
    while (out - buffer < buflen - 1) {
        if (!*infostring || *infostring == '\\') break;
        *out++ = *infostring++;
    }
    *out = 0;
...
void Info_SetValueForStarKey(char *infostring, const char *key, const char *value, int maxsize) {
    ...
    if (*oldvalue) {
        ...
        if (len > maxsize - 1) goto ErrTooLong;
    }
    ...
    if (len > sizeof(buffer) - 1) goto ErrTooLong;
    ...
    while (*info) {
        ...
        if (strcasecmp(key, "name")) {
            ...
            /* auto lowercase team */
            if (!strcasecmp(key, "team")) c = tolower(c);
        }
#else
...
int build_number(void) {
    ...
    for (m = 0; m < 11; m++) {
        if (strncasecmp(&date[0], mon[m], 3) == 0) break;
        d += mond[m];
    }
