...
struct stree_root *Cvar_ArgCompletions(const char *name, const char *buf) {
    ...
    if (cvar && cvar->completion) root = cvar->completion(buf);
...
void Cvar_Set(const char *var_name, const char *value) {
    ...
    if (changed && var->callback) var->callback(var);

#ifdef NQ_HACK
    // Don't allow deathmatch and coop at the same time...
    if ((var->value != 0) && (!strcmp(var->name, deathmatch.name))) Cvar_Set("coop", "0");
    if ((var->value != 0) && (!strcmp(var->name, coop.name))) Cvar_Set("deathmatch", "0");
#endif
}
