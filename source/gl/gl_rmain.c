...
cvar_t r_fullbright = {.name = "r_fullbright", .string = "0", .flags = CVAR_DEVELOPER};
cvar_t gl_keeptjunctions = {.name = "gl_keeptjunctions", .string = "1", .flags = CVAR_OBSOLETE};
cvar_t gl_reporttjunctions = {.name = "gl_reporttjunctions", .string = "0", .flags = CVAR_OBSOLETE};
cvar_t gl_texsort = {.name = "gl_texsort", .string = "1", .flags = CVAR_OBSOLETE};
...
int R_SpriteDataSize(int numpixels) { return offsetof(gl_spritedata_t, pixels[numpixels]); }
...
=================
*/
static void R_DrawSpriteModel(const entity_t *entity) {
...
static void GL_FloodFillSkin(byte *skin, int skinwidth, int skinheight) {
...
static void GL_LoadSkinData(model_t *model, aliashdr_t *aliashdr, const alias_skindata_t *skindata) {
...
const model_loader_t *R_ModelLoader(void) { return &GL_Model_Loader; }
...
=============
*/
static void GL_AliasDrawModel(const entity_t *entity, float blend) {
    aliashdr_t *aliashdr;
...
=============
*/
static void GL_AliasDrawShadow(const entity_t *entity, aliashdr_t *paliashdr, int posenum) {
    trivertx_t *verts;
    int *order;
    vec3_t point;
...
===============
*/
static void R_AliasSetupSkin(const entity_t *entity, aliashdr_t *aliashdr) {
...
=================
*/
static void R_AliasSetupFrame(entity_t *entity, aliashdr_t *aliashdr) {
...
=================
*/
static void R_AliasCalcLerp(entity_t *entity, vec3_t origin, vec3_t angles) {
#ifdef NQ_HACK
...
static void R_AliasCalcLight(const entity_t *entity, const vec3_t origin, const vec3_t angles) {
    ...
    } else if (!strcmp(model->name, "progs/flame.mdl") || !strcmp(model->name, "progs/flame2.mdl")) {
        // HACK HACK HACK -- no fullbright colors, so make torches full
        // light
...
=================
*/
static void R_AliasDrawModel(entity_t * entity) {
    const model_t *model = entity->model;
    ...
    if (!strcmp(model->name, "progs/eyes.mdl")) {
        ...
        glScalef(aliashdr->scale[0] * 2, aliashdr->scale[1] * 2, aliashdr->scale[2] * 2);
    } else {
        glTranslatef(aliashdr->scale_origin[0], aliashdr->scale_origin[1], aliashdr->scale_origin[2]);
        glScalef(aliashdr->scale[0], aliashdr->scale[1], aliashdr->scale[2]);
    }
