 ...
 */
static void R_AddDynamicLights(msurface_t *surf, unsigned *blocklights) {
    int lnum;
    int sd, td;
...
static void DrawGLPolyLM(glpoly_t *p) {
    int i;
    float *v;
...
static void DrawGLPoly_2Ply(glpoly_t *p) {
    int i;
    float *v;
...
static void WaterWarpCoord(vec3_t in, vec3_t out) {
...
static void DrawFlatGLPoly(glpoly_t *p) {
    int i;
    float *v;
 ...
 */
static void R_UploadLMBlockUpdate(int blocknum) {
    glRect_t *rect;
    byte *pixels;
    unsigned offset;
 ...
 */
static void R_UpdateLightmapBlockRect(msurface_t *fa) {
    int map;
    byte *base;
    int smax, tmax;
    lm_block_t *block;
    ...
    foreach_surf_lightstyle(fa, map) if (d_lightstylevalue[fa->styles[map]] != fa->cached_light[map]) goto dynamic;
...
================
*/
static void DrawTextureChains(const entity_t *e) {
    int i;
    msurface_t *s;
    texture_t *t;
...
static void DrawFlatTextureChains(void) {
    int i;
    msurface_t *s;
    texture_t *t;
    ...
    if (!r_drawflat.value) {
        ...
        if (brushmodel->firstmodelsurface != 0 && !gl_flashblend.value) {
            for (k = 0; k < MAX_DLIGHTS; k++) {
                ...
                R_MarkLights(&cl_dlights[k], 1 << k, brushmodel->nodes + brushmodel->hulls[0].firstclipnode);
            }
        }
    }
...
static void R_RecursiveWorldNode(mnode_t *node) {
    ...
    if (c) {
        ...
        for (; c; c--, surf++) {
            ...
            if (!WATER_WARP_TEST(surf) && ((dot < 0) ^ !!(surf->flags & SURF_PLANEBACK)))
                continue; // wrong side
...
================
*/
static void BuildSurfaceDisplayList(brushmodel_t *brushmodel, msurface_t *surf, void *hunkbase) {
