...
static void SubdividePolygon(msurface_t *surf, int numverts, vec_t *verts, const char *hunkname) {
    int i, j, k, memsize;
    vec3_t mins, maxs;
    float m;
    float *v;
    ...
    for (i = 0; i < 3; i++) {
        ...
        for (j = 0; j < numverts; j++, v += 3) {
            ...
            if ((dist[j] > 0) != (dist[j + 1] > 0)) {
                ...
                for (k = 0; k < 3; k++)
                    front[front_count][k] = back[back_count][k] = v[k] + frac * (v[3 + k] - v[k]);
                front_count++;
                back_count++;
            }
        }
...
=================
*/
void EmitSkyPolysMtex(msurface_t *fa) {
    glpoly_t *p;
    float *v;
    int i;
    float s, t;
