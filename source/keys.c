...
static void ShowCompletions(void) {
    const char *s;
...
static void EnterCommand(const char *buf) {
    /*
    ...
    if (buf[0] == '\\' || buf[0] == '/')
        ...
        if (cls.state >= ca_connected) Cbuf_AddText("say ");
        Cbuf_AddText("%s", buf);
    }
    Cbuf_AddText("\n");
}
...
====================
*/
static void Key_Console(knum_t key) {
#ifdef _WIN32
    int i;
    HANDLE th;
...
static void Key_Message(knum_t key) {
    if (key == K_ENTER) {
        if (chat_team)
...
===================
*/
static knum_t Key_StringToKeynum(const char *name) {
    const keyname_t *keyname;

    if (!name || !name[0]) return K_UNKNOWN;
    if (!name[1]) return (unsigned char)name[0];

    for (keyname = keynames; keyname->name; keyname++) {
        if (!strcasecmp(name, keyname->name)) return keyname->keynum;
    }
...
const char *Key_KeynumToString(knum_t keynum) {
    ...
    if (keynum == K_UNKNOWN) return "<KEY NOT FOUND>";
    ...
    for (keyname = keynames; keyname->name; keyname++)
        if (keynum == keyname->keynum) return keyname->name;
...
static void Key_Unbindall_f(void) {
    ...
    for (keynum = K_UNKNOWN + 1; keynum < K_LAST; keynum++)
        if (keybindings[keynum]) Key_SetBinding(keynum, NULL);
}
...
void Key_WriteBindings(FILE *f) {
    ...
    if (in_cfg_unbindall.value) fprintf(f, "unbindall\n");
...
void Key_Init(void) {
    ...
    //
    for (keynum = K_SPACE; keynum <= K_DEL; keynum++) consolekeys[keynum] = true;
    ...
    for (keynum = K_UNKNOWN; keynum < K_LAST; keynum++) keyshift[keynum] = keynum;
    for (keynum = 'a'; keynum <= 'z'; keynum++) keyshift[keynum] = keynum - 'a' + 'A';
    ...
    for (keynum = K_F1; keynum <= K_F15; keynum++) menubound[keynum] = true;
...
void Key_Event(knum_t key, qboolean down) {
    ...
    if (down) {
        key_repeats[key]++;
        if (key != K_BACKSPACE && key != K_PAUSE && key != K_PGUP && key != K_PGDN &&
            key_repeats[key] > 1)
    ...
    if (key == K_LSHIFT) lshift_down = down;
    if (key == K_RSHIFT) rshift_down = down;
    ...
    if (!down) {
        ...
        if (binding && binding[0] == '+') Cbuf_AddText("-%s %i\n", binding + 1, key);
        if (keyshift[key] != key) {
    ...
    if ((key_dest == key_menu && menubound[key]) ||
        ...
        binding = keybindings[key];
        if (binding) {
            if (binding[0] == '+') /* button commands add keynum as a parm */
                Cbuf_AddText("%s %i\n", binding, key);
            else
                Cbuf_AddText("%s\n", binding);
        }
    ...
    if (lshift_down || rshift_down) key = keyshift[key];
