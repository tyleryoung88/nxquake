...
static void M_DrawTransPic(int x, int y, const qpic8_t *pic) {
    Draw_TransPic(x + ((vid.width - 320) >> 1), y, pic);
}

void M_DrawPic(int x, int y, const qpic8_t *pic) { Draw_Pic(x + ((vid.width - 320) >> 1), y, pic); }
...
static void M_BuildTranslationTable(int top, int bottom) {
    ...
    for (j = 0; j < 256; j++) identityTable[j] = j;
...
static void M_DrawTransPicTranslate(int x, int y, const qpic8_t *pic) {
...
static void M_FindKeysForCommand(m_keys_cursor_t commandnum, knum_t keys[2]) {
...
static void M_UnbindCommand(m_keys_cursor_t commandnum) {
    const char *command, *binding;
    int length;
    knum_t keynum;
    ...
    for (keynum = K_UNKNOWN + 1; keynum < K_LAST; keynum++) {
        binding = keybindings[keynum];
        if (!binding) continue;
        if (!strncmp(binding, command, length)) Key_SetBinding(keynum, NULL);
    }
}
...
static void M_Keys_Key(knum_t keynum) {
    ...
    switch (keynum) {
        ...
        case K_ENTER:
            ...
            if (keys[1] != K_UNKNOWN) M_UnbindCommand(m_keys_cursor);
...
static void M_Video_Key(knum_t keynum) { (*vid_menukeyfn)(keynum); }
...
static void M_Help_Draw(void) { M_DrawPic(0, 0, Draw_CachePic(va("gfx/help%i.lmp", help_page))); }
...
static void M_LanConfig_Draw(void) {
    ...
    if (*m_return_reason) M_PrintWhite(basex, 148, m_return_reason);
}
...
static void M_LanConfig_Key(knum_t keynum) {
    ...
    switch (keynum) {
        ...
        default:
            ...
            if (keynum < K_0 || keynum > K_9) break;
            if (lanConfig_cursor == 0) {
...
static void M_GameOptions_Draw(void) {
    ...
    if (hipnotic) {
        M_Print(160, 120,
                hipnoticlevels[hipnoticepisodes[startepisode].firstLevel + startlevel].description);
        M_Print(160, 128,
    ...
    // PGM 01/07/97 added rogue episodes
        ...
        M_Print(160, 120,
                roguelevels[rogueepisodes[startepisode].firstLevel + startlevel].description);
