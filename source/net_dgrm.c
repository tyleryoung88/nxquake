...
#ifdef DEBUG
static const char *StrAddr(netadr_t *addr) {
    static char buf[32];

    snprintf(buf, sizeof(buf), "%d.%d.%d.%d:%d", addr->ip.b[0], addr->ip.b[1], addr->ip.b[2],
             addr->ip.b[3], ntohs(addr->port));
    return buf;
}
#endif
...
static int SendPacket(qsocket_t *sock) {
...
int Datagram_SendMessage(qsocket_t *sock, const sizebuf_t *data) {
    ...
    if (sock->canSend == false) Sys_Error("%s: called with canSend == false", __func__);
#endif
...
int Datagram_SendUnreliableMessage(qsocket_t *sock, const sizebuf_t *data) {
    ...
    if (data->cursize > sock->mtu) Sys_Error("%s: message too big %u", __func__, data->cursize);
#endif
...
int Datagram_GetMessage(qsocket_t *sock) {
    ...
    while (1) {
        length = sock->landriver->Read(sock->socket, &packetBuffer, NET_MESSAGESIZE, &readaddr);
#if 0
    ...
    if ((rand() & 255) > 220)
        ...
        if (flags & NETFLAG_DATA) {
            ...
            sock->landriver->Write(sock->socket, &packetBuffer, NET_HEADERSIZE, &readaddr);
...
void NET_Stats_f(void) {
    ...
    if (Cmd_Argc() == 1) {
        ...
        for (s = net_activeSockets; s; s = s->next) PrintStats(s);
        for (s = net_freeSockets; s; s = s->next) PrintStats(s);
    } else {
        for (s = net_activeSockets; s; s = s->next)
            if (strcasecmp(Cmd_Argv(1), s->address) == 0) break;
        if (s == NULL)
...
static void Test_Poll(struct test_poll_state *state) {
    netadr_t clientaddr;
    int control;
    int len;
    char *name;
    ...
    while (1) {
        len =
            state->driver->Read(state->socket, net_message.data, net_message.maxsize, &clientaddr);
        ...
        Con_Printf("%s (%d)\n  frags:%3i  colors:%u %u  time:%u\n  %s\n", name, (int)playerNumber,
                   frags, colors >> 4, colors & 0x0f, connectTime / 60, address);
    }
...
static void Test_f(void) {
    ...
    static struct test_poll_state state = {
        .inProgress = false, .pollCount = 0, .socket = 0, .driver = NULL, .procedure = NULL};
    static PollProcedure poll_procedure = {
        .next = NULL, .nextTime = 0.0, .procedure = Test_Poll, .arg = &state};
    ...
    if (host && hostCacheCount) {
        for (n = 0; n < hostCacheCount; n++)
            if (strcasecmp(host, hostcache[n].name) == 0) {
                if (hostcache[n].driver != dgrm_driver) continue;
                driver = hostcache[n].ldriver;
                max = hostcache[n].maxusers;
                sendaddr = hostcache[n].addr;
                break;
            }
        if (driver) goto JustDoIt;
    }
    ...
    for (i = 0; i < net_numlandrivers; i++) {
        ...
        if (driver->GetAddrFromName(host, &sendaddr) != -1) break;
    }
    if (!driver) return;

JustDoIt:
    state.socket = driver->OpenSocket(0);
    if (state.socket == -1) return;
...
static void Test2_Poll(struct test_poll_state *state) {
    netadr_t clientaddr;
    int control;
    int len;
    char *name;
    ...
    if (len < sizeof(int)) goto Reschedule;

    net_message.cursize = len;

    MSG_BeginReading();
    control = MSG_ReadControlHeader();
    if (control == -1) goto Error;
    if ((control & (~NETFLAG_LENGTH_MASK)) != NETFLAG_CTL) goto Error;
    if ((control & NETFLAG_LENGTH_MASK) != len) goto Error;

    if (MSG_ReadByte() != CCREP_RULE_INFO) goto Error;
    ...
    MSG_WriteControlHeader(&net_message);
    state->driver->Write(state->socket, net_message.data, net_message.cursize, &clientaddr);
    SZ_Clear(&net_message);
...
static void Test2_f(void) {
    ...
    static struct test_poll_state state = {
        .inProgress = false, .pollCount = 0, .socket = 0, .driver = NULL, .procedure = NULL};
    static PollProcedure poll_procedure = {
        .next = NULL, .nextTime = 0.0, .procedure = Test2_Poll, .arg = &state};
    ...
    if (host && hostCacheCount) {
        ...
        if (state.driver) goto JustDoIt;
    }

    for (i = 0; i < net_numlandrivers; i++) {
        if (!net_landrivers[i].initialized) continue;
        // see if we can resolve the host name
...
void Datagram_Close(qsocket_t *sock) { sock->landriver->CloseSocket(sock->socket); }

void Datagram_Listen(qboolean state) {
    int i;

    for (i = 0; i < net_numlandrivers; i++)
        if (net_landrivers[i].initialized) net_landrivers[i].Listen(state);
}
...
static qsocket_t *_Datagram_CheckNewConnections(net_landriver_t *driver) {
    ...
    if (command == CCREQ_SERVER_INFO) {
        if (strcmp(MSG_ReadString(), "QUAKE") != 0) return NULL;
    ...
    if (command == CCREQ_PLAYER_INFO) {
        ...
        if (clientNumber == svs.maxclients) return NULL;
    ...
    for (s = net_activeSockets; s; s = s->next) {
        if (s->driver != net_driver) continue;
...
qsocket_t *Datagram_CheckNewConnections(void) {
    ...
    for (i = 0; i < net_numlandrivers; i++) {
        ...
        if (driver->initialized)
            if ((ret = _Datagram_CheckNewConnections(driver)) != NULL) break;
    }
...
static void _Datagram_SearchForHosts(qboolean xmit, net_landriver_t *driver) {
    ...
    while ((ret = driver->Read(driver->controlSock, net_message.data, net_message.maxsize,
                               &readaddr)) > 0) {
        if (ret < sizeof(int)) continue;
        net_message.cursize = ret;

        // don't answer our own query
        if (NET_AddrCompare(&readaddr, &myaddr) >= 0) continue;
        ...
        for (i = 0, host = hostcache; i < hostCacheCount; i++, host++)
            if (NET_AddrCompare(&readaddr, &host->addr) == 0) break;
        hostnum = i;

        // is it already there?
        if (hostnum < hostCacheCount) continue;
        ...
        for (i = 0; i < hostCacheCount; i++) {
            ...
            if (!strcasecmp(host->name, hostcache[i].name)) {
                 ...
                 */
                if (len < max - 1 && host->name[len - 1] > '0' + HOSTCACHESIZE) {
                    host->name[len] = '0';
                    host->name[len + 1] = 0;
                } else
                    host->name[len - 1]++;
...
void Datagram_SearchForHosts(qboolean xmit) {
    ...
    for (i = 0; i < net_numlandrivers; i++) {
        ...
        driver = &net_landrivers[i];
        if (driver->initialized) _Datagram_SearchForHosts(xmit, driver);
    }
}
...
static qsocket_t *_Datagram_Connect(const char *host, net_landriver_t *driver) {
    ...
    // see if we can resolve the host name
    if (driver->GetAddrFromName(host, &sendaddr) == -1) return NULL;
...
qsocket_t *Datagram_Connect(const char *host) {
    ...
    for (i = 0; i < net_numlandrivers; i++) {
        driver = &net_landrivers[i];
        if (driver->initialized)
            if ((ret = _Datagram_Connect(host, driver)) != NULL) break;
    }
    return ret;
}
